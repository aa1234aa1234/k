...
public class Player {
    ...
    public void setPosition(Point pos) { position = pos; location = new Point(pos.x,pos.y); collisionbox.setLocation((int) (location.x-(int)collisionbox.getWidth()/2.0), (int) (location.y-(int)collisionbox.getHeight()/2.0)); System.out.println(collisionbox); }
...
//                return false;
//            }
            for(int y = (hitBox.y)/Room.getTileSize(); y<=(hitBox.y+hitBox.getHeight()*2)/Room.getTileSize(); y++) {
                if(room.getMap()[y][hitBox.x/Room.getTileSize()].getVal() && hitBox.intersects(room.getMap()[y][hitBox.x/Room.getTileSize()].getArea())) return false;
                else if(room.getMap()[y][(int) ((hitBox.x+hitBox.getWidth())/Room.getTileSize())].getVal() && hitBox.intersects(room.getMap()[y][(int) ((hitBox.x+hitBox.getWidth())/Room.getTileSize())].getArea())) return false;
            }
            for(int x = (hitBox.x)/Room.getTileSize(); x<=(hitBox.x+hitBox.getWidth())/Room.getTileSize(); x++) {
                if(room.getMap()[hitBox.y/Room.getTileSize()][x].getVal() && hitBox.intersects(room.getMap()[hitBox.y/Room.getTileSize()][x].getArea())) return false;
                else if(room.getMap()[(int) (hitBox.y+hitBox.getHeight()*2)/Room.getTileSize()][x].getVal() && hitBox.intersects(room.getMap()[(int) (hitBox.y+hitBox.getHeight()*2)/Room.getTileSize()][x].getArea())) return false;
            }
        }
