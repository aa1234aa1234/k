...
public class Room {
    ...
    private ArrayList<Pair<Asset,Integer>> assets = new ArrayList<>();
    ...
    public void addAsset(Asset asset, int layer) {
        ...
        asset.getCollision().x -= asset.getCollision().width/2.0;
        asset.getCollision().y -= asset.getSize().height/2.0;
        assets.add(Pair.of(asset,layer));
        //loadAssets();
    }

    public void loadAssets() {
        for(Pair<Asset,Integer> a : assets) {
            if(a.getFirst().getImage() == null) continue;
            this.layer.get(a.getSecond()).getGraphics().drawImage(a.getFirst().getImage(),a.getFirst().getSize().x,a.getFirst().getSize().y,a.getFirst().getSize().width,a.getFirst().getSize().height,null);
        }

    }

    public void testlol() {
        for(BufferedImage a : layer) {
            a = new BufferedImage(image.getWidth(),image.getHeight(),BufferedImage.TYPE_INT_ARGB);
        }
        loadAssets();
    }
    ...
    public ArrayList<Pair<Asset,Integer>> getAssets() {
        return assets;
    }

    public void setAssets(ArrayList<Pair<Asset,Integer>> assets) {
        this.assets = assets;
    }
    ...
    public void setCutScene(CutScene a) {
        cutScene = a;
    }
