...
public class Game extends JFrame {
    private GameLayer gamepanel;
    private JPanel UILayer;
    private DialogBox dialogBox = new DialogBox();
    private Thread mainthread,gamethread;
    ...
    public void initialize() {
        player.setInteraction(new Interaction() {
            ...
            public void interact() {
                ...
                for(Pair<Asset,Integer> e : rooms.get(currentRoom).getAssets()) {
                    if(player.getInteractionBox().intersects(e.getFirst().getCollision())) {
                        e.getFirst().doInteraction();
                    }
                }
            }
        ...
        addKeyListener(new KeyListener() {
            ...
            public void keyPressed(KeyEvent e) {
                ...
                if(dialogBox.isVisible()) {
                    if(e.getKeyCode() == KeyEvent.VK_Z) {
                        dialogBox.whenyoupresstheinteractkeywhendialogboxisopen();
                    }
                    if(e.getKeyCode() == KeyEvent.VK_X) {
                        dialogBox.whenyoupressthenotinteractkeywhilethedialogboxisvisible();
                    }
                    return;
                }
                switch(e.getKeyCode()) {
                ...
                if(player.isWalking() == 0 && e.getKeyCode() >= 37 && e.getKeyCode() <= 40) {
                    //player.ChangeDir(e.getKeyCode()-37);

                    player.ChangeDir((Integer) keylist.toArray()[0]-37);
                }
            ...
            public void keyReleased(KeyEvent e) {
                ...
                if(keylist.size() != 0) player.ChangeDir((Integer) keylist.toArray()[0]-37);
            }
        });
        ...
        gamepanel.setRender(new IRender() {
            ...
            public void render(Graphics2D graphics2D) {
                ...
                graphics2D.drawImage(rooms.get(currentRoom).getLayer().get(0),getImageXStart(),getImageYStart(),getImageXEnd(),getImageYEnd(),getXViewPortStart(),getYViewPortStart(),getXViewPortEnd(),getYViewPortEnd(),null);
                graphics2D.drawImage(player.getImage(),(int)(player.getPosition().x-34.0),(int)(player.getPosition().y-39.0),68,78,null);
                for(int i = 1; i<rooms.get(currentRoom).getLayer().size(); i++) {
                    graphics2D.drawImage(rooms.get(currentRoom).getLayer().get(i),getImageXStart(),getImageYStart(),getImageXEnd(),getImageYEnd(),getXViewPortStart(),getYViewPortStart(),getXViewPortEnd(),getYViewPortEnd(),null);
                }
            }
        });
    ...
    public void gamevalue() throws IOException {
        ...
        rooms.add(new Room("maps/map.png",new Point(1500,350),3));
        rooms.add(new Room("map_collision/bruh.png", new Point(709,1000),3));
        //rooms.add(new Room("maps/lol.png", new Point(500,350),3));
        rooms.add(new rm_test());
        //rooms.get(0).getAssets().add(new Main.Exit(70,300,1560,260) {{ setNextRoomNo(1); setEnterPoint(new Point(100,350)); }});
        loadRoomJson();
        rooms.get(1).loadCollision(ImageIO.read(new File("map_collision/bruh.png")));
        rooms.get(2).loadCollision(rooms.get(2).getImage());
        loadAssetJson();
//        rooms.get(0).addAsset(new Main.Asset(315,250,350,120,"assets/house.png"),1);
//        rooms.get(2).addAsset(new Main.Asset(70,100,500,350,"player_sprites/(baka).png"),1);
        ...
        try {
            ...
            for(int i = 0; i<array.length(); i++) {
                ...
                for (int j = 0; j < exitsArray.length(); j++) {
                    ...
                    rooms.get(obj.getInt("roomNo")).addExit(new Exit(exitObject.getInt("Width"), exitObject.getInt("Height"), exitObject.getInt("x"), exitObject.getInt("y")) {{ setNextRoomNo(exitObject.getInt("NextRoomNo")); setEnterPoint(new Point(enterPoint.getInt("x"),enterPoint.getInt("y"))); }});
                }
            }
        } catch (IOException e) {
    ...
    private void loadAssetJson() {
        try {
            JSONObject json = new JSONObject(new String(Files.readAllBytes(Paths.get("assets.json"))));
            JSONArray array = json.getJSONArray("assets");
            for(int i = 0; i<array.length(); i++) {
                JSONObject obj = array.getJSONObject(i);
                JSONObject collision = obj.getJSONObject("Collision"), size = obj.getJSONObject("Size");
                int Width = collision.getInt("Width"), Height = collision.getInt("Height"), x = collision.getInt("x"), y = collision.getInt("y");
                Rectangle rect = new Rectangle(size.getInt("x"),size.getInt("y"),size.getInt("Width"),size.getInt("Height"));
                String imagepath = obj.getString("ImagePath");
                int layer = obj.getInt("Layer");
                rooms.get(obj.getInt("roomNo")).addAsset(new Asset(Width,Height,x,y,rect,imagepath),layer);
                System.out.println(Width + " " + Height + " " + x + " " + y + " " + rect);
            }
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    ...
    public void otherstuff() throws InterruptedException{
        mainthread = new Thread(new Runnable() {
            @Override
            public void run() {
                long currentTime = System.currentTimeMillis(),deltaTime = 0,lastTime = 0;
                boolean flag = false;
                while(true) {
                    try {
                        ...
                        if(rooms.get(currentRoom).getCutScene() != null) rooms.get(currentRoom).getCutScene().playCutScene();
                        if(!dialogBox.isVisible()) {
                            if (player.move(keydown.isLeft(), keydown.isRight(), keydown.isUp(), keydown.isBottom(), getXViewPortStart(), getYViewPortStart(), getXViewPortEnd(), getYViewPortEnd(), rooms.get(currentRoom).getImage().getWidth(), rooms.get(currentRoom).getImage().getHeight(), rooms.get(currentRoom))) {
                                ...
                                if(flag || player.isWalking() == 0) {
                                    player.setWalking(player.isWalking() % 2== 0 ? 1 : 2);
                                    flag = false;
                                }
                            }
                            else {
                                if(player.isWalking() != 0) player.setWalking(0);
                            }
                        }
                        ...
                        gamepanel.repaint();
                        //assetlayer.repaint();
                        lastTime += System.currentTimeMillis()-currentTime;
                        if(lastTime >= 300) {
                            flag = true;
                            lastTime = 0;
                        }
    ...
    private void objectData() {
        obj_test obj_test = new obj_test();
        obj_test.setIt(new obj_test.Interaction() {
            @Override
            public void onInteract() {
                if(obj_test.getFlag()) return;
                dialogBox.printText("* howdy im bob.",player.getPosition());
                dialogBox.printText("* thats about all i have to say",player.getPosition());
                obj_test.setAnimationTimer(new Timer(300,e -> {
                    if(dialogBox.isQueueEmpty()) ((Timer)e.getSource()).stop();
                    if(dialogBox.isPrintDone() && obj_test.getAnimationFrame() == 0) return;
                    obj_test.setAnimationFrame((obj_test.getAnimationFrame()+1)%2);
                }));
                obj_test.getAnimationTimer().setRepeats(true);
                obj_test.getAnimationTimer().start();
                obj_test.setFlag(true);
            }
        });
        rooms.get(2).addAsset(obj_test,0);
        rooms.get(2).setCutScene(new Room.CutScene() {
            @Override
            public void playCutScene() {
                if(player.getLocation().y < 500) {
                    obj_test.doInteraction();
                }
            }
        });
    }
